<template>
  <div class="p-6 space-y-6">
    ...
    <div v-if="loading" class="text-center py-20 dark:text-white">
      ...
      <p class="mt-2 text-gray-500">Sincronizando catálogo...</p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <fwb-card v-for="p in allProperties" :key="p.id" class="flex flex-col h-full overflow-hidden border-gray-200 dark:border-gray-700 relative">
        ...
        <div class="absolute top-2 left-2 z-10">
          <div v-if="p.ownerId" class="bg-purple-600 text-white text-[10px] px-2 py-1 rounded-full flex items-center gap-1 shadow-md uppercase font-bold">
        ...
        <div class="p-5 flex-1 flex flex-col">
          ...
          <div class="flex justify-between items-end mt-auto">
            <div>
              <p class="text-xs text-gray-400 uppercase font-bold tracking-tighter">Precio Lista</p>
    ...
    <div v-if="!loading && allProperties.length === 0" class="text-center py-20 bg-gray-50 dark:bg-gray-800 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700">
       <p class="text-gray-500">No hay inmuebles que coincidan con los filtros.</p>
    </div>

    <fwb-modal v-if="showOpTypeModal" @close="showOpTypeModal = false">
      <template #header><div class="text-lg font-bold">Cambiar Categoría de Operación</div></template>
      <template #body>
        <div class="space-y-4">
          <p class="text-sm text-gray-500">Inmueble: <strong>{{ selectedProp?.title }}</strong></p>
          <div>
            <label class="block mb-2 text-sm font-medium">Nueva Categoría</label>
            <select v-model="newOpType" class="w-full bg-gray-50 border border-gray-300 text-sm rounded-lg p-2.5 dark:bg-gray-700 dark:text-white">
              <option value="VENTA">Venta</option>
              <option value="ALQUILER">Alquiler</option>
              <option value="ANTICRETICO">Anticrético</option>
            </select>
      ...
      <template #footer>
        <div class="flex justify-end space-x-2">
          <fwb-button color="alternative" @click="showOpTypeModal = false">Cancelar</fwb-button>
          <fwb-button @click="doOpTypeUpdate" gradient="blue">Guardar Cambios</fwb-button>
        </div>
      </template>
    </fwb-modal>

    <property-details-modal v-if="showDetailsModal" :show="showDetailsModal" :property="selectedProp" @close="showDetailsModal = false" />
    ...
    <fwb-modal v-if="showPriceModal" @close="showPriceModal = false">
      ...
      <template #footer>
        <div class="flex justify-end space-x-2">
          ...
          <fwb-button @click="doPriceUpdate" gradient="blue">Guardar Precio</fwb-button>
        </div>
      </template>
    </fwb-modal>
    ...
    <fwb-modal v-if="showOwnerModal" @close="showOwnerModal = false">
      ...
      <template #body>
        <div class="space-y-4">
          ...
          <select v-model="selectedOwnerId" class="w-full border border-gray-300 rounded-lg p-2.5 dark:bg-gray-700 dark:text-white">
            <option value="">Sin propietario</option>
            <option v-for="owner in availableOwners" :key="owner.id" :value="owner.id">{{ owner.fullName }} ({{ owner.email }})</option>
          </select>
        </div>
      </template>
      <template #footer>
...
const showOpTypeModal = ref(false) // Nuevo

// Datos de selección
const selectedProp = ref<any>(null)
const newPrice = ref(0)
const newOpType = ref('') // Nuevo
...
const load = async () => {
  loading.value = true
  try {
    // Usamos el backend con soporte para filtros dinámicos
    const params: any = {}
    if (filterTitle.value) params.title = filterTitle.value
    if (filterOpType.value) params.status = filterOpType.value // Se mapea según tu lógica de findWithFilters

    const [p, u] = await Promise.all([
      api.get('/properties', { params: { 
        title: filterTitle.value || undefined, 
        status: filterOpType.value || undefined // Ajustado para que el backend filtre por operación si envías el param correcto
      }}),
      userService.getUsers()
    ])
    allProperties.value = p.data
    allUsers.value = u
  } catch (e) {
...
const getOwnerName = (ownerId: string) => {
  ...
  return owner ? (owner.fullName || owner.name) : 'Cargando...'
}
...
// --- ACCIONES DE MODALES ---
const viewDetails = (p: any) => { selectedProp.value = p; showDetailsModal.value = true }
const openCreateModal = () => { selectedProp.value = null; showCreateModal.value = true }
...
const handleCreate = async (data: any) => {
  try {
    ...
    alert("Error: " + (e.response?.data?.detail || "No se pudo registrar"))
  }
}
...
// Actualización de Categoría/Operación (LO QUE FALTABA)
const prepOpTypeUpdate = (p: any) => {
  selectedProp.value = p
  newOpType.value = p.operationType
  showOpTypeModal.value = true
}

const doOpTypeUpdate = async () => {
  try {
    // Llamada al nuevo endpoint PATCH /properties/{id}/operation-type
    ...
    alert(e.response?.data?.detail || "Error al cambiar categoría")
  }
}
...
const doAssign = async (agentId: string) => {
  try {
    ...
    await load()
  } catch (e: any) { alert(e.response?.data?.detail || "Error en asignación") }
}
...
const doAssignOwner = async () => {
  try {
    ...
    await load()
  } catch (e: any) { alert("Error al asignar propietario") }
}
...
<style scoped>
.fwb-card { transition: transform 0.2s ease, box-shadow 0.2s ease; }
.fwb-card:hover { transform: translateY(-4px); box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); }
</style>