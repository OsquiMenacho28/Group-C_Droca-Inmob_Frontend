<template>
  <div class="p-6 space-y-6">
    ...
    >
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        ...
        <div>
          ...
          >
            <option value="">
              {{ t('clientProperties.allZonesOption', 'Todas las zonas') }}
            </option>
      ...
      >
        <template #actions-top="{ property, statusHelpers }">
          <button
            ...
            :disabled="
              statusHelpers.isSold.value || statusHelpers.isDeleted.value
            "
    ...
    <fwb-modal v-if="showOpTypeModal" @close="showOpTypeModal = false">
      ...
      <template #footer>
        <fwb-button @click="doOpTypeUpdate" gradient="blue">{{
          t('common.save')
        }}</fwb-button>
      </template>
    </fwb-modal>
    ...
    <fwb-modal v-if="showDeleteModal" @close="showDeleteModal = false">
      <template #header>
        <div class="text-lg font-bold text-red-600">
          {{ t('adminProperties.deleteProperty') }}
        </div>
      </template>
      <template #body>
        <p class="dark:text-gray-300">
          {{
            t('agentDashboard.confirmDeleteText', {
...
const openEditModal = (property: Property) => {
  if (property.status === 'ELIMINADO') {
    toast.message =
      'No se puede editar una propiedad eliminada. Reincorpórala primero.';
    toast.type = 'error';
    toast.show = true;
    return;
  }
...
const prepOwnerAssignment = (p: Property) => {
  if (p.status === 'ELIMINADO') {
    toast.message =
      'No se puede asignar propietario a una propiedad eliminada.';
    toast.type = 'error';
    toast.show = true;
    return;
  }
...
const prepPriceUpdate = (p: Property) => {
  if (p.status === 'ELIMINADO') {
    toast.message =
      'No se puede modificar el precio de una propiedad eliminada.';
    toast.type = 'error';
    toast.show = true;
    return;
  }
...
const prepOpTypeUpdate = (p: Property) => {
  if (p.status === 'ELIMINADO') {
    toast.message =
      'No se puede modificar el tipo de operación de una propiedad eliminada.';
    toast.type = 'error';
    toast.show = true;
    return;
  }
...
const prepClosure = (p: Property) => {
  if (p.status === 'ELIMINADO' || p.status === 'RETIRADO') {
    toast.message =
      'No se puede registrar cierre de una propiedad eliminada o retirada.';
    toast.type = 'error';
    toast.show = true;
    return;
  }
...
const handleReincorporate = async () => {
  ...
  try {
    ...
    const index = allProperties.value.findIndex(
