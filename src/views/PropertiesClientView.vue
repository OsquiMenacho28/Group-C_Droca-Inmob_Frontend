<template>
  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    ...
    <div class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 transition-colors">
      <div class="max-w-7xl mx-auto">
        <div class="flex items-center gap-3">
          ...
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Propiedades disponibles</h1>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 transition-colors">
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-3">
          ...
          <!-- Precio mínimo -->
          <div>
            <label class="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Precio mínimo ($)</label>
            <input
          ...
          <!-- Precio máximo -->
          <div>
            <label class="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Precio máximo ($)</label>
            <input
          ...
          <!-- Tipo -->
          <div>
            <label class="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Tipo</label>
            <select
      ...
      <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="i in 6" :key="i" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 h-64 animate-pulse transition-colors">
      ...
      <div v-else-if="error" class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl p-4 text-red-700 dark:text-red-400 text-sm transition-colors">
        {{ error }} <button @click="loadProperties" class="ml-2 underline hover:text-red-800 dark:hover:text-red-300">Reintentar</button>
      </div>

      <div v-else-if="properties.length === 0" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center transition-colors">
        ...
        <p class="text-gray-500 dark:text-gray-400 font-medium">No hay propiedades disponibles con esos filtros.</p>
        <button @click="clearFilters" class="mt-2 text-sm text-blue-600 dark:text-blue-400 underline">Ver todas</button>
      </div>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        >
          <!-- Imagen / placeholder -->
          <div class="relative h-44 bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-gray-700 dark:to-gray-600 flex items-center justify-center transition-colors">
            <img v-if="prop.imageUrl" :src="prop.imageUrl" :alt="prop.name" class="w-full h-full object-cover" />
            <svg v-else class="h-16 w-16 text-blue-300 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
            </svg>
            <!-- Badge Disponible -->
            <span class="absolute top-3 left-3 bg-green-500 dark:bg-green-600 text-white text-xs font-semibold px-2 py-1 rounded-full shadow-sm">
              Disponible
            </span>
            <!-- Tipo -->
            <span class="absolute top-3 right-3 bg-white/90 dark:bg-gray-900/80 text-gray-700 dark:text-gray-200 text-xs font-medium px-2 py-1 rounded-full backdrop-blur-sm">
              {{ prop.type }}
            </span>
          </div>
          ...
          <!-- Info -->
          <div class="p-4">
            <h3 class="font-semibold text-gray-900 dark:text-white truncate">{{ prop.name }}</h3>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate">📍 {{ prop.address }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Zona: {{ prop.zone }}</p>
            ...
            <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">Agente: {{ prop.agentName }}</p>
          </div>
        </div>
      </div>
    ...
    <Transition name="fade">
      ...
      >
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden transition-colors">
          <div class="h-48 bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-gray-700 dark:to-gray-600 flex items-center justify-center transition-colors">
            <img v-if="selectedProperty.imageUrl" :src="selectedProperty.imageUrl" class="w-full h-full object-cover" />
            <svg v-else class="h-20 w-20 text-blue-300 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
            </svg>
          </div>
          <div class="p-6">
            <div class="flex items-start justify-between">
              <div>
                <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ selectedProperty.name }}</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{{ selectedProperty.address }}</p>
              </div>
              <button @click="selectedProperty = null" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-2 gap-3 mt-4 text-sm">
              <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-3 transition-colors">
                <p class="text-xs text-gray-500 dark:text-gray-400">Precio</p>
                <p class="font-bold text-blue-700 dark:text-blue-400 text-base">${{ selectedProperty.price.toLocaleString() }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-3 transition-colors">
                <p class="text-xs text-gray-500 dark:text-gray-400">Tipo</p>
                <p class="font-medium text-gray-900 dark:text-white">{{ selectedProperty.type }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-3" v-if="selectedProperty.bedrooms">
                <p class="text-xs text-gray-500 dark:text-gray-400">Habitaciones</p>
                <p class="font-medium text-gray-900 dark:text-white">{{ selectedProperty.bedrooms }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-3" v-if="selectedProperty.area">
                <p class="text-xs text-gray-500 dark:text-gray-400">Área (m²)</p>
                <p class="font-medium text-gray-900 dark:text-white">{{ selectedProperty.area }}</p>
              </div>
            </div>
            <p v-if="selectedProperty.description" class="text-sm text-gray-600 dark:text-gray-300 mt-3">
    ...
    <Transition name="fade">
      ...
      >
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg p-6 overflow-y-auto max-h-[90vh] transition-colors">

          <div class="flex items-start justify-between mb-4">
            <div>
              <h2 class="text-lg font-bold text-gray-900 dark:text-white">Solicitar visita</h2>
              <p class="text-sm text-gray-500 dark:text-gray-400">{{ requestTarget.name }}</p>
            </div>
            <button @click="closeRequestModal" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
              <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
              </svg>
            </button>
          ...
          <form @submit.prevent="submitVisitRequest" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Tu nombre *</label>
                <input
                  v-model="requestForm.clientName"
                  placeholder="Nombre completo"
                  class="w-full rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
                  :class="{ 'border-red-400 dark:border-red-500': reqErrors.clientName }"
                  required
                />
              ...
              <div>
                ...
                <input
                  ...
                  class="w-full rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
                  :class="{ 'border-red-400 dark:border-red-500': reqErrors.clientEmail }"
                  required
                />
            ...
            <div>
              <label class="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Teléfono (opcional)</label>
              <input
            ...
            <div>
              <label class="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Horario preferido *</label>
              <input
                ...
                :class="{ 'border-red-400 dark:border-red-500': reqErrors.preferredDateTime }"
                :min="minDatetime"
                required
              />
            ...
            <div>
              <label class="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Horario alternativo (opcional)</label>
              <input
            ...
            <div>
              <label class="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Mensaje para el agente</label>
              <textarea
            ...
            <p class="text-xs text-gray-500 dark:text-gray-300 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-lg px-3 py-2">
              📨 Al enviar esta solicitud, <strong class="dark:text-amber-200">{{ requestTarget.agentName }}</strong>
          ...
          <!-- Éxito -->
          <div v-if="requestSuccess" class="mt-4 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-xl p-4 text-center transition-colors">
            <svg class="h-8 w-8 text-green-500 dark:text-green-400 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
            <p class="text-sm font-semibold text-green-800 dark:text-green-200">¡Solicitud enviada!</p>
            <p class="text-xs text-green-700 dark:text-green-300 mt-1">El agente {{ requestTarget?.agentName }} ha sido notificado.</p>
            <button @click="closeRequestModal" class="mt-3 text-xs text-green-700 dark:text-green-300 underline">Cerrar</button>
          </div>
        </div>
      </div>
    </Transition>
