    ...
    >
      <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
        ...
        <div>
          ...
          >
            <option :value="6">
              {{ t('agentDashboard.itemsCount', { n: 6 }) }}
            </option>
            <option :value="12">
              {{ t('agentDashboard.itemsCount', { n: 12 }) }}
            </option>
            <option :value="24">
              {{ t('agentDashboard.itemsCount', { n: 24 }) }}
            </option>
          </select>
        </div>
    ...
    <fwb-modal v-if="showDeleteModal" @close="showDeleteModal = false">
      <template #header>
        <div class="text-lg font-bold text-red-600">
          {{ t('agentDashboard.confirmDelete') }}
        </div>
      </template>
      <template #body>
        <div class="space-y-4">
      ...
      <template #footer>
        <div class="flex justify-end space-x-2">
          ...
          >
            {{
              deleting
                ? t('agentDashboard.deleting')
                : t('agentDashboard.deletePermanently')
            }}
          </fwb-button>
        </div>
      </template>
...
const load = async () => {
  ...
  try {
    ...
    console.error(
      t('common.error') + ' ' + t('nav.myInmuebles').toLowerCase() + ':',
      e
    );
  } finally {
    loading.value = false;
...
const doDeleteProperty = async () => {
  ...
  try {
    ...
    const axiosError = e as AxiosError<ApiErrorResponse>;
    console.error(
      t('common.error') + ' ' + t('common.delete').toLowerCase() + ' property:',
      e
    );
