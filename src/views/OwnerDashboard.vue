<template>
  <div class="p-6 space-y-6">
    ...
    <fwb-tabs v-model="activeTab" variant="underline" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm">
      <fwb-tab name="properties" :title="t('ownerDashboard.propertiesTab')">
        <div class="mt-6">
          ...
          <div v-else-if="errorProps" class="bg-red-50 border border-red-200 rounded-lg p-4 text-red-700">
            {{ errorProps }}
          </div>
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="prop in properties" :key="prop.id" @click="openDetails(prop)" class="cursor-pointer">
              <fwb-card class="overflow-hidden hover:shadow-lg transition-shadow relative h-full">
                <div class="absolute top-4 right-4 z-10">
                  <div class="bg-blue-600 text-white rounded-full px-3 py-1 text-sm font-bold flex items-center gap-1 shadow-lg">
                    ...
                    {{ prop.visitCount || 0 }} {{ t('ownerDashboard.visits', { n: prop.visitCount || 0 }) }}
                  </div>
                </div>
                <div class="h-48 bg-gray-100 dark:bg-gray-700 flex items-center justify-center relative">
                  <img v-if="prop.imageUrls && prop.imageUrls.length > 0" :src="prop.imageUrls[0]" class="h-full w-full object-cover" />
                  <div v-else class="text-gray-400 flex flex-col items-center">
                    <IconLucideImage class="w-12 h-12 mb-2" />
                    <span class="text-sm">{{ t('common.noImage') }}</span>
                  </div>
                  <div class="absolute top-4 left-4">
                    <span :class="statusBadgeClass(prop.status)" class="px-2 py-1 rounded-full text-xs font-semibold shadow-md">
                ...
                <div class="p-5">
                  ...
                  <div class="grid grid-cols-2 gap-2 text-sm text-gray-600 dark:text-gray-300 border-t border-gray-100 dark:border-gray-700 pt-3">
                    <div class="flex items-center gap-1"><span>📐</span> {{ prop.m2 }} m²</div>
                    <div class="flex items-center gap-1"><span>🛏️</span> {{ prop.rooms }} {{ t('users.view.rooms') }}</div>
                    <div class="flex items-center gap-1 col-span-2">
                      <span>👤</span> {{ t('common.agent') }} {{ prop.assignedAgentName || prop.assignedAgentId || t('common.unassigned') }}
                    </div>
                  </div>
                </div>
              </fwb-card>
        ...
        <div class="mt-6">
          <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
            ...
            <div v-if="loadingNotif" class="space-y-4">
              <div v-for="i in 3" :key="i" class="animate-pulse h-20 bg-gray-100 dark:bg-gray-700 rounded-xl"></div>
            </div>
...
<script setup lang="ts">
  ...
  const statusBadgeClass = (status: string) => {
    switch (status) {
      ...
      case 'EN_NEGOCIACION': return 'bg-blue-500 text-white';
